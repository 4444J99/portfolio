...
```
src/components/
├── Header.astro              # Sticky nav, mobile hamburger, inline toggle script (is:inline)
├── Footer.astro
├── ProjectCard.astro          # Typed card for project grids
├── ProjectDetail.astro        # Individual project page layout
├── sketches/
│   └── SketchContainer.astro  # p5.js canvas wrapper (data-sketch attr loads specific sketch)
└── academic/                  # Technical writing components
...
**No CSS framework.** One global stylesheet (`src/styles/global.css`, ~600 lines) defines the entire design system via CSS custom properties:
...
`src/data/` contains JSON files (projects, essays, about, landing, graph, system-metrics) generated by `npm run generate-data` from a sibling repo. These are imported at build time by Astro pages.
