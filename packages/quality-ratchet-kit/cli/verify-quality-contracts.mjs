...
if (metrics.tests.total !== vitestReport.numTotalTests) {
	fail(
		`tests.total mismatch: metrics=${metrics.tests.total}, report=${vitestReport.numTotalTests}`,
	);
}
if (metrics.tests.passed !== vitestReport.numPassedTests) {
	fail(
		`tests.passed mismatch: metrics=${metrics.tests.passed}, report=${vitestReport.numPassedTests}`,
	);
}
...
if (metrics.security.githubOpenAlerts !== githubOpenAlerts) {
	fail(
		`security.githubOpenAlerts mismatch: metrics=${metrics.security.githubOpenAlerts}, summary=${githubOpenAlerts}`,
	);
}
if (metrics.security.githubAdvisoryStatus !== securityGithubSummary.status) {
	fail(
		`security.githubAdvisoryStatus mismatch: metrics=${metrics.security.githubAdvisoryStatus}, summary=${securityGithubSummary.status}`,
	);
}
...
for (const [field, expected] of prodPairs) {
	if (metrics.security.prodCounts[field] !== expected) {
		fail(
			`security.prodCounts.${field} mismatch: metrics=${metrics.security.prodCounts[field]}, summary=${expected}`,
		);
	}
}
...
const expectedDevCounts = {
	critical:
		typeof fullCounts.critical === 'number' && typeof prodCounts.critical === 'number'
			? Math.max(0, fullCounts.critical - prodCounts.critical)
			: null,
	high:
		typeof fullCounts.high === 'number' && typeof prodCounts.high === 'number'
			? Math.max(0, fullCounts.high - prodCounts.high)
			: null,
	moderate:
		typeof fullCounts.moderate === 'number' && typeof prodCounts.moderate === 'number'
			? Math.max(0, fullCounts.moderate - prodCounts.moderate)
			: null,
	low:
		typeof fullCounts.low === 'number' && typeof prodCounts.low === 'number'
			? Math.max(0, fullCounts.low - prodCounts.low)
			: null,
	total:
		typeof fullCounts.total === 'number' && typeof prodCounts.total === 'number'
			? Math.max(0, fullCounts.total - prodCounts.total)
			: null,
};
for (const field of ['critical', 'high', 'moderate', 'low', 'total']) {
	if (metrics.security.devCounts[field] !== expectedDevCounts[field]) {
		fail(
			`security.devCounts.${field} mismatch: metrics=${metrics.security.devCounts[field]}, expected=${expectedDevCounts[field]}`,
		);
	}
}
...
if (
	...
	typeof metrics.security.policyCheckpoint.date !== 'string'
) {
...
const registerCheckpoints = Array.isArray(securityRegister.checkpoints)
	? securityRegister.checkpoints
	: [];
...
for (const [field, expected] of staticPairs) {
	if (metrics.a11y.static[field] !== expected) {
		fail(
			`a11y.static.${field} mismatch: metrics=${metrics.a11y.static[field]}, summary=${expected}`,
		);
	}
}
...
for (const [field, expected] of runtimePairs) {
	if (metrics.a11y.runtime[field] !== expected) {
		fail(
			`a11y.runtime.${field} mismatch: metrics=${metrics.a11y.runtime[field]}, summary=${expected}`,
		);
	}
}
...
if ((runtimeErrors?.counts?.uncategorized ?? 0) > 0) {
	fail(
		`runtime error telemetry uncategorized errors detected: ${runtimeErrors.counts.uncategorized}`,
	);
}

if (metrics.runtimeErrors.status !== runtimeErrors.status) {
	fail(
		`runtimeErrors.status mismatch: metrics=${metrics.runtimeErrors.status}, summary=${runtimeErrors.status}`,
	);
}
if (metrics.runtimeErrors.total !== toNullableNumber(runtimeErrors?.counts?.total)) {
	fail(
		`runtimeErrors.total mismatch: metrics=${metrics.runtimeErrors.total}, summary=${toNullableNumber(runtimeErrors?.counts?.total)}`,
	);
}
if (
	metrics.runtimeErrors.uncategorized !== toNullableNumber(runtimeErrors?.counts?.uncategorized)
) {
	fail(
		`runtimeErrors.uncategorized mismatch: metrics=${metrics.runtimeErrors.uncategorized}, summary=${toNullableNumber(runtimeErrors?.counts?.uncategorized)}`,
	);
}
if (metrics.runtimeErrors.allowlisted !== toNullableNumber(runtimeErrors?.counts?.allowlisted)) {
	fail(
		`runtimeErrors.allowlisted mismatch: metrics=${metrics.runtimeErrors.allowlisted}, summary=${toNullableNumber(runtimeErrors?.counts?.allowlisted)}`,
	);
}

if (metrics.stability.status !== greenRuns.status) {
	fail(
		`stability.status mismatch: metrics=${metrics.stability.status}, summary=${greenRuns.status}`,
	);
}
if (metrics.stability.consecutiveSuccess !== toNullableNumber(greenRuns.consecutiveSuccess)) {
	fail(
		`stability.consecutiveSuccess mismatch: metrics=${metrics.stability.consecutiveSuccess}, summary=${toNullableNumber(greenRuns.consecutiveSuccess)}`,
	);
}
if (metrics.stability.requiredConsecutive !== toNullableNumber(greenRuns.requiredConsecutive)) {
	fail(
		`stability.requiredConsecutive mismatch: metrics=${metrics.stability.requiredConsecutive}, summary=${toNullableNumber(greenRuns.requiredConsecutive)}`,
	);
}
if (
	metrics.stability.history.length !== (Array.isArray(greenRuns.runs) ? greenRuns.runs.length : 0)
) {
...
if (metrics.ledger.generated !== qualityLedger.generated) {
	fail(
		`ledger.generated mismatch: metrics=${metrics.ledger.generated}, ledger=${qualityLedger.generated}`,
	);
}
if (
	metrics.ledger.snapshots.length !==
	(Array.isArray(qualityLedger.snapshots) ? qualityLedger.snapshots.length : 0)
) {
...
if (metrics.performance.routeBudgetsStatus !== perfBudget.routeBudgetsStatus) {
	fail(
		`performance.routeBudgetsStatus mismatch: metrics=${metrics.performance.routeBudgetsStatus}, summary=${perfBudget.routeBudgetsStatus}`,
	);
}
if (metrics.performance.chunkBudgetsStatus !== perfBudget.chunkBudgetsStatus) {
	fail(
		`performance.chunkBudgetsStatus mismatch: metrics=${metrics.performance.chunkBudgetsStatus}, summary=${perfBudget.chunkBudgetsStatus}`,
	);
}
if (metrics.performance.interactionBudgetsStatus !== perfBudget.interactionBudgetsStatus) {
	fail(
		`performance.interactionBudgetsStatus mismatch: metrics=${metrics.performance.interactionBudgetsStatus}, summary=${perfBudget.interactionBudgetsStatus}`,
	);
}
...
if ((metrics.performance.routeBudgetCheckpoint?.date ?? null) !== routeCheckpointDate) {
	fail(
		`performance.routeBudgetCheckpoint mismatch: metrics=${metrics.performance.routeBudgetCheckpoint?.date ?? null}, summary=${routeCheckpointDate}`,
	);
}
if ((metrics.performance.chunkBudgetCheckpoint?.date ?? null) !== chunkCheckpointDate) {
	fail(
		`performance.chunkBudgetCheckpoint mismatch: metrics=${metrics.performance.chunkBudgetCheckpoint?.date ?? null}, summary=${chunkCheckpointDate}`,
	);
}
if ((metrics.performance.interactionBudgetCheckpoint?.date ?? null) !== interactionCheckpointDate) {
	fail(
		`performance.interactionBudgetCheckpoint mismatch: metrics=${metrics.performance.interactionBudgetCheckpoint?.date ?? null}, summary=${interactionCheckpointDate}`,
	);
}

const expectedLargestChunks = Array.isArray(perfSummary.largestChunks)
	? perfSummary.largestChunks
			.slice(0, 10)
			.map((chunk) => ({ chunk: chunk.chunk, gzipBytes: chunk.gzipBytes }))
	: [];
...
if (
	JSON.stringify(metrics.performance.routeJsTotals) !==
	JSON.stringify(perfSummary.routeJsTotals ?? {})
) {
	fail('performance.routeJsTotals mismatch with perf summary');
}
if (
	JSON.stringify(metrics.performance.interactiveRouteJsTotals) !==
	JSON.stringify(perfSummary.interactiveRouteJsTotals ?? {})
) {
	fail('performance.interactiveRouteJsTotals mismatch with perf summary');
}

const lhrFiles = readdirSync(paths.lighthouseDir).filter(
...
if (!lhrFiles.length) {
	...
	for (const file of lhrFiles) {
		...
		if (lhr.categories?.performance?.score !== undefined)
			scores.performance.push(lhr.categories.performance.score * 100);
		if (lhr.categories?.accessibility?.score !== undefined)
			scores.accessibility.push(lhr.categories.accessibility.score * 100);
		if (lhr.categories?.['best-practices']?.score !== undefined)
			scores.bestPractices.push(lhr.categories['best-practices'].score * 100);
	...
	for (const [metric, expected] of lhPairs) {
		if (metrics.lighthouse[metric] !== expected) {
			fail(
				`lighthouse.${metric} mismatch: metrics=${metrics.lighthouse[metric]}, recomputed=${expected}`,
			);
		}
	}
}
...
if (metrics.build.bundleFiles !== bundleCount) {
	fail(
		`build.bundleFiles mismatch: metrics=${metrics.build.bundleFiles}, dist/_astro=${bundleCount}`,
	);
}
...
if (!Number.isFinite(metricsGenerated)) {
	...
	if (freshnessLagMs > 120000) {
		fail(
			`metrics are stale by ${(freshnessLagMs / 1000).toFixed(1)}s; regenerate with npm run generate-badges`,
		);
	}
}
