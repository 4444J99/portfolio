...
async function fetchWithRetry(
	url,
	createRequest,
	{ maxAttempts, baseDelayMs, maxDelayMs, retryableStatus = shouldRetryStatus },
) {
	let retryCount = 0;
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		...
		try {
			...
			const request =
				built && typeof built === 'object' && 'request' in built ? built.request : built;
...
async function fetchPaginatedArray(url, headers, runtime) {
	...
	while (nextUrl) {
		const result = await fetchWithRetry(nextUrl, () => ({ request: { headers } }), {
...
async function fetchJsonObject(url, headers, runtime) {
	const result = await fetchWithRetry(url, () => ({ request: { headers } }), {
		maxAttempts: runtime.retryAttempts,
		baseDelayMs: runtime.backoffBaseMs,
		maxDelayMs: runtime.backoffMaxMs,
	});
...
async function fetchWithRetryAndTimeout(
	url,
	{ method, headers, timeoutMs, maxAttempts, baseDelayMs, maxDelayMs },
) {
...
function writeFallbackEnvelope(outputPath, existingPayload, errors, stats, logger) {
	if (!existingPayload || typeof existingPayload !== 'object' || Array.isArray(existingPayload)) {
		logger.warn(
			`Unable to annotate fallback metadata in ${outputPath}; existing payload is invalid.`,
		);
		return;
	}
	...
	const updated = {
		...
		stats: {
			...
			typeof existingPayload.stats === 'object' &&
			!Array.isArray(existingPayload.stats)
				? existingPayload.stats
				: {}),
			...stats,
