...
const SUPPORTED_SCHEMA_VERSIONS = new Set(['github-pages-index.v2', 'github-pages-index.v2.1']);
...
export function validateGitHubPagesIndex({
	...
	if (payload?.stats !== undefined) {
		if (!isObject(payload.stats)) {
			...
			if (payload.stats.fallbackAt !== undefined && !isFiniteDateString(payload.stats.fallbackAt)) {
	...
	for (let index = 0; index < repos.length; index += 1) {
		...
		if (!isNullableString(repo.sourceBranch))
			errors.push(`${prefix}.sourceBranch must be string|null.`);
		if (!isNullableString(repo.sourcePath))
			errors.push(`${prefix}.sourcePath must be string|null.`);
		...
		if (!isNullableNumber(repo.httpStatus))
			errors.push(`${prefix}.httpStatus must be number|null.`);
		if (typeof repo.reachable !== 'boolean') errors.push(`${prefix}.reachable must be boolean.`);
		if (!isNullableString(repo.redirectTarget))
			errors.push(`${prefix}.redirectTarget must be string|null.`);
	...
	if (stale) {
		errors.push(`Data is stale: generatedAt is ${ageHours.toFixed(2)}h old (max ${maxAgeHours}h).`);
	}
