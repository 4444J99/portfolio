...
export default function tokenStreamSketch(p: p5, container: HTMLElement) {
	...
	p.draw = () => {
		...
		tokens.forEach((token) => {
			...
			if (token.labelIdx >= 0 && tokenLabels[token.labelIdx] && !isMobile()) {
				...
				p.text(
					tokenLabels[token.labelIdx].substring(0, 18),
					token.x + 2,
		...
		if (selectedInfo && selectedInfo.alpha > 0) {
			const statusText = selectedInfo.verified ? 'Factual: VERIFIED' : 'Hallucinated: flagged';
			const [sr, sg, sb] = selectedInfo.verified
				? PALETTE.accent
				: ([180, 80, 80] as [number, number, number]);
	...
	(p as p5 & { touchStarted?: () => boolean | void }).touchStarted = () => {
		handleClick();
		return false;
	};
