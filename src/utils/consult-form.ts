...
export const ORGAN_PROFILES: OrganProfile[] = [
  ...
  },
  {
    ...
    keywords: ['art', 'creative', 'installation', 'music', 'experience', 'interactive', 'performance'],
  },
  {
    id: 'III',
    title: 'ERGON',
...
export function sanitizeHtml(html: string): string {
  return html.replace(/<\/?([a-zA-Z][a-zA-Z0-9]*)\b([^>]*)>/g, (tag: string, name: string, attrs: string) => {
    ...
    const safeAttrs = (attrs.match(/\s[\w-]+="[^"]*"/g) || [])
      .filter((attr: string) => allowedAttrs.some((allowed: string) => attr.trimStart().startsWith(`${allowed}=`)));
...
export function buildFallbackAnalysis(
  ...
  html += '<li>Map that slice to the listed organ capabilities and lock a governance workflow before implementation.</li>';
  html += '<li>Email <code>padavano.anthony@gmail.com</code> with your challenge context for an architecture review.</li>';
  html += '</ul>';
