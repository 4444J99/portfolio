...
import { classificationColors, getChartTheme } from './chart-theme';
...
export default function flagshipStacked(container: HTMLElement, data: { repos: Repo[] }) {
	...
	g.selectAll('.bar-code')
		...
		.on('mousemove', (event, d) => {
			tooltip.show(
				`<strong>${d.repo}</strong><br/>Code: ${d.code_files}, Tests: ${d.test_files}<br/>${d.classification}`,
				event,
			);
		})
	...
	g.selectAll('.bar-test')
		...
		.on('mousemove', (event, d) => {
			tooltip.show(
				`<strong>${d.repo}</strong><br/>Code: ${d.code_files}, Tests: ${d.test_files}`,
				event,
			);
		})
		.on('mouseleave', () => tooltip.hide());
	...
	g.append('g')
		...
		.call((g) =>
			g.selectAll('.tick text').attr('fill', theme.textMuted).style('font-size', '0.65rem'),
		);

	// Legend
	const legend = svg.append('g').attr('transform', `translate(${margin.left}, ${height - 10})`);
	[
		{ label: 'Code', color: theme.accent },
		{ label: 'Tests', color: classificationColors.SUBSTANTIAL },
	].forEach((item, i) => {
		legend
			.append('rect')
			.attr('x', i * 80)
