...
export default function organBarChart(container: HTMLElement, data: { organs: OrganData[] }) {
	...
	g.selectAll('rect')
		...
		.on('mousemove', (event, d) => {
			tooltip.show(
				`<strong>${d.name}</strong><br/>${d.total_repos} repos, ${d.ci_coverage} CI`,
				event,
			);
		})
	...
	g.append('g')
		...
		.call(
			d3.axisBottom(x).tickFormat((d) => d.replace('ORGAN-', '').replace('META-ORGANVM', 'META')),
		)
		.call((g) => g.select('.domain').attr('stroke', theme.border))
		.call((g) => g.selectAll('.tick line').remove())
		.call((g) =>
			g.selectAll('.tick text').attr('fill', theme.textMuted).style('font-size', '0.7rem'),
		);
