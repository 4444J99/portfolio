...
if (securityFullSummary) {
	...
	metrics.security.status =
		typeof securityFullSummary.status === 'string' ? securityFullSummary.status : 'unknown';
	metrics.security.allowlistActive =
		toNumberOrNull(securityFullSummary?.allowlist?.activeCount) ??
		(Array.isArray(securityFullSummary?.allowlist?.active)
			? securityFullSummary.allowlist.active.length
			: 0);
...
if (securityGithubSummary) {
	metrics.security.githubAdvisoryStatus =
		typeof securityGithubSummary.status === 'string' ? securityGithubSummary.status : 'unknown';
	...
	if (metrics.security.status === 'unknown') {
		...
		metrics.security.status === 'pass' &&
		metrics.security.githubAdvisoryStatus === 'fail'
	) {
...
if (existsSync(staticA11yPath)) {
	...
	metrics.a11y.static.status =
		typeof staticA11y.status === 'string' ? staticA11y.status : 'unknown';
}
...
if (existsSync(runtimeA11yPath)) {
	...
	metrics.a11y.runtime.status =
		typeof runtimeA11y.status === 'string' ? runtimeA11y.status : 'unknown';
}
...
if (existsSync(perfBudgetPath)) {
	...
	metrics.performance.routeBudgetsStatus =
		typeof perfBudget.routeBudgetsStatus === 'string'
			? perfBudget.routeBudgetsStatus
			: typeof perfBudget.status === 'string'
				? perfBudget.status
				: 'unknown';
	metrics.performance.chunkBudgetsStatus =
		typeof perfBudget.chunkBudgetsStatus === 'string' ? perfBudget.chunkBudgetsStatus : 'unknown';
	metrics.performance.interactionBudgetsStatus =
		typeof perfBudget.interactionBudgetsStatus === 'string'
			? perfBudget.interactionBudgetsStatus
			: 'unknown';
	metrics.performance.routeBudgetCheckpoint =
		perfBudget?.routeThresholdCheckpoint &&
		typeof perfBudget.routeThresholdCheckpoint.date === 'string'
			? { date: perfBudget.routeThresholdCheckpoint.date }
			: null;
	metrics.performance.chunkBudgetCheckpoint =
		perfBudget?.chunkThresholdCheckpoint &&
		typeof perfBudget.chunkThresholdCheckpoint.date === 'string'
			? { date: perfBudget.chunkThresholdCheckpoint.date }
			: null;
	metrics.performance.interactionBudgetCheckpoint =
		perfBudget?.interactionThresholdCheckpoint &&
		typeof perfBudget.interactionThresholdCheckpoint.date === 'string'
...
if (existsSync(runtimeErrorsPath)) {
	...
	metrics.runtimeErrors.status =
		typeof runtimeErrors.status === 'string' ? runtimeErrors.status : 'unknown';
...
const a11yStates = [metrics.a11y.static.status, metrics.a11y.runtime.status].filter(
	(state) => state !== 'unknown',
);
metrics.a11y.status =
	a11yStates.length === 0
		? 'unknown'
		: a11yStates.every((state) => state === 'pass')
			? 'pass'
			: 'fail';
...
const testsBadge =
	metrics.tests.total === null || metrics.tests.passed === null
...
writeFileSync(
	...
	badge(
		'coverage',
		coverageScore === null ? 'n/a' : `${coverageScore}%`,
		scoreColor(coverageScore),
	),
);
writeFileSync(
...
writeFileSync(
	...
	badge(
		'SEO',
		metrics.lighthouse.seo === null ? 'n/a' : `${metrics.lighthouse.seo}`,
		scoreColor(metrics.lighthouse.seo),
	),
);
writeFileSync(
	join(badgesDir, 'a11y.svg'),
	badge('a11y', metrics.a11y.status, statusColor(metrics.a11y.status)),
);
writeFileSync(
	join(badgesDir, 'security.svg'),
	badge('security', metrics.security.status, statusColor(metrics.security.status)),
);
writeFileSync(
	join(badgesDir, 'perf-budgets.svg'),
	badge('perf-budgets', perfBudgetOverall, statusColor(perfBudgetOverall)),
);
