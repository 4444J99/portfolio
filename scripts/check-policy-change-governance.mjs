...
if (!baseSha || !headSha) {
	...
	summary.failures.push(
		'Missing base/head SHA context; governance check applies only to pull_request runs.',
	);
...
summary.checks.securityPolicyCheckbox = hasChecked(
	prBody,
	/\[[xX]\]\s*Security policy impact reviewed/i,
);
summary.checks.securityAllowlistCheckbox = hasChecked(
	prBody,
	/\[[xX]\]\s*Security allowlist impact reviewed/i,
);
...
if (changedSecurityPolicy && !summary.checks.securityPolicyCheckbox) {
	summary.failures.push(
		'Security policy files changed but PR checklist does not confirm policy impact review.',
	);
}

if (changedSecurityPolicy && !summary.checks.securityAllowlistCheckbox) {
	summary.failures.push(
		'Security policy files changed but PR checklist does not confirm allowlist impact review.',
	);
}

if (changedPerfPolicy && !summary.checks.perfBudgetCheckbox) {
	summary.failures.push(
		'Performance policy files changed but PR checklist does not confirm budget regression review.',
	);
}
