...
lines.push('');
lines.push(
	`- Typecheck: ${typecheckSummary?.status ?? 'unknown'} (hints ${typecheckSummary?.hints ?? 'n/a'} / budget ${hintTarget ?? 'n/a'})`,
);
lines.push(
	`- Security: ${current?.security?.status ?? 'n/a'} (critical ${current?.security?.critical ?? 'n/a'}, high ${current?.security?.high ?? 'n/a'})`,
);
lines.push(
	`- Security split: prod ${current?.security?.prodCounts?.total ?? 'n/a'} / dev ${current?.security?.devCounts?.total ?? 'n/a'} (allowlist active ${current?.security?.allowlistActive ?? 'n/a'})`,
);
lines.push(
	`- Security checkpoint: ${current?.security?.policyCheckpoint ? `${current.security.policyCheckpoint.date} (moderate<=${current.security.policyCheckpoint.maxModerate}, low<=${current.security.policyCheckpoint.maxLow})` : 'n/a'}`,
);
lines.push(
	`- Coverage: statements ${current?.coverage?.statements ?? 'n/a'}, branches ${current?.coverage?.branches ?? 'n/a'}, functions ${current?.coverage?.functions ?? 'n/a'}, lines ${current?.coverage?.lines ?? 'n/a'}`,
);
if (coverageTarget) {
	lines.push(
		`- Coverage targets (${phase}): ${coverageTarget.statements}/${coverageTarget.branches}/${coverageTarget.functions}/${coverageTarget.lines}`,
	);
}
lines.push(
	`- Accessibility: static ${current?.a11y?.static?.status ?? 'n/a'}, runtime ${current?.a11y?.runtime?.status ?? 'n/a'}, overall ${current?.a11y?.status ?? 'n/a'}`,
);
lines.push(
...
lines.push(
	`- Runtime errors: ${runtimeErrorsSummary?.status ?? current?.runtimeErrors?.status ?? 'n/a'} (uncategorized ${runtimeErrorsSummary?.counts?.uncategorized ?? current?.runtimeErrors?.uncategorized ?? 'n/a'})`,
);
const greenRunStatus =
	greenRunsSummary?.status === 'skipped'
		? 'Local Skip (Missing GITHUB_TOKEN)'
		: (greenRunsSummary?.status ?? current?.stability?.status ?? 'unknown');
lines.push(
	`- Green-run tracker: ${greenRunStatus} (${greenRunsSummary?.consecutiveSuccess ?? current?.stability?.consecutiveSuccess ?? 'n/a'}/${greenRunsSummary?.requiredConsecutive ?? current?.stability?.requiredConsecutive ?? 'n/a'} consecutive)`,
);
lines.push(
	`- Perf budgets: route ${current?.performance?.routeBudgetsStatus ?? 'n/a'}, chunk ${current?.performance?.chunkBudgetsStatus ?? 'n/a'}, interaction ${current?.performance?.interactionBudgetsStatus ?? 'n/a'}`,
);
lines.push(
	`- Lighthouse: perf ${current?.lighthouse?.performance ?? 'n/a'}, a11y ${current?.lighthouse?.accessibility ?? 'n/a'}, bp ${current?.lighthouse?.bestPractices ?? 'n/a'}, seo ${current?.lighthouse?.seo ?? 'n/a'}`,
);
lines.push('');
lines.push('## Security Sprint Cadence');
lines.push('');

const checkpoints = Array.isArray(securityRegister?.checkpoints)
...
if (checkpoints.length > 0) {
	...
	checkpoints.forEach((checkpoint) => {
		lines.push(
			`| ${checkpoint.date ?? 'n/a'} | ${formatSeverityTarget(checkpoint.target)} | ${checkpoint.status ?? 'n/a'} | ${checkpoint.notes ?? ''} |`,
		);
	});
} else {
...
lines.push('');
const families = Array.isArray(securityRegister?.vulnerabilityFamilies)
	? securityRegister.vulnerabilityFamilies
	: [];
if (families.length > 0) {
	families.forEach((family) => {
		lines.push(
			`- ${family.family ?? 'unknown'}: owner ${family.owner ?? 'n/a'}, ETA ${family.eta ?? 'n/a'}, status ${family.status ?? 'n/a'}`,
		);
	});
} else {
	lines.push('- None');
...
if (rollbackPolicy) {
	...
	const requirements = Array.isArray(rollbackPolicy.requirements)
		? rollbackPolicy.requirements
		: [];
...
lines.push('|---|---:|---:|---:|');
lines.push(
	metricRow('Coverage statements', current?.coverage?.statements, baseline?.coverage?.statements),
);
lines.push(
	metricRow('Coverage branches', current?.coverage?.branches, baseline?.coverage?.branches),
);
lines.push(
	metricRow('Coverage functions', current?.coverage?.functions, baseline?.coverage?.functions),
);
lines.push(metricRow('Coverage lines', current?.coverage?.lines, baseline?.coverage?.lines));
lines.push(
	metricRow('Security critical', current?.security?.critical, baseline?.security?.critical),
);
lines.push(metricRow('Security high', current?.security?.high, baseline?.security?.high));
lines.push(
	metricRow('Security moderate', current?.security?.moderate, baseline?.security?.moderate),
);
lines.push(metricRow('Security low', current?.security?.low, baseline?.security?.low));
lines.push(
	metricRow(
		'Security github open alerts',
		current?.security?.githubOpenAlerts,
		baseline?.security?.githubOpenAlerts,
	),
);
lines.push(
	metricRow('LH performance', current?.lighthouse?.performance, baseline?.lighthouse?.performance),
);
lines.push(
	metricRow(
		'LH accessibility',
...
lines.push(
	metricRow(
		'LH best practices',
		current?.lighthouse?.bestPractices,
		baseline?.lighthouse?.bestPractices,
	),
);
...
lines.push(
	metricRow(
		'Runtime coverage %',
		current?.a11y?.runtime?.coveragePct,
		baseline?.a11y?.runtime?.coveragePct,
	),
);
lines.push(
	metricRow(
		'Runtime uncategorized errors',
		current?.runtimeErrors?.uncategorized,
		baseline?.runtimeErrors?.uncategorized,
	),
);
lines.push(
	metricRow(
...
lines.push(
	metricRow(
		'A11y serious total',
		(current?.a11y?.static?.serious ?? 0) + (current?.a11y?.runtime?.serious ?? 0),
