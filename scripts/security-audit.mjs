...
const checkpointRules = {
	maxModerate: Number.isFinite(checkpoint.maxModerate)
		? checkpoint.maxModerate
		: Number.POSITIVE_INFINITY,
	maxLow: Number.isFinite(checkpoint.maxLow) ? checkpoint.maxLow : Number.POSITIVE_INFINITY,
};

const allowlistPolicy = {
	maxDurationDays: Number.isFinite(policy.allowlist?.maxDurationDays)
		? policy.allowlist.maxDurationDays
		: 14,
...
for (const entry of allowlistEntries) {
	...
	if (createdAt !== null && expiresAt !== null && expiresAt < createdAt)
		validationErrors.push('expires precedes created');
	if (!['critical', 'high', 'moderate', 'low', 'info'].includes(severity))
		validationErrors.push(`invalid severity: ${severity}`);
	if (durationMs !== null && durationMs > allowlistPolicy.maxDurationDays * 24 * 60 * 60 * 1000) {
...
function matchAllowlist(vulnerability) {
	return (
		activeAllowlistEntries.find(
			(entry) =>
				entry.package === vulnerability.package && entry.severity === vulnerability.severity,
		) ?? null
	);
}
...
if (unsuppressedCounts.critical > rules.maxCritical) {
	failures.push(
		`unsuppressed critical vulnerabilities ${unsuppressedCounts.critical} exceed max ${rules.maxCritical}`,
	);
}
if (unsuppressedCounts.high > rules.maxHigh) {
	failures.push(
		`unsuppressed high vulnerabilities ${unsuppressedCounts.high} exceed max ${rules.maxHigh}`,
	);
}
if (scope === 'full') {
...
if (summary.status === 'fail') {
	...
	unsuppressed
		.filter((vulnerability) =>
			['critical', 'high', 'moderate', 'low'].includes(vulnerability.severity),
		)
