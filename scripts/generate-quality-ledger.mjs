...
const securityDriftPath = resolve(
	parseOption('security-drift', '.quality/security-drift-summary.json'),
);
...
const snapshot = {
	...
	drift: {
		...
		securityDriftFailures: Array.isArray(securityDrift?.failures)
			? securityDrift.failures.length
			: null,
...
lines.push(
	`- Current consecutive green runs: ${fmt(snapshot.stability.consecutiveSuccess)} / ${fmt(snapshot.stability.requiredConsecutive)}`,
);
lines.push('');
lines.push(
	'| Generated | Security | Runtime A11y % | Perf Budgets | Coverage (S/B/F/L) | LH Perf | Green Runs |',
);
lines.push('|---|---|---:|---|---|---:|---:|');
